...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def saveLabels(self, annotationFilePath, annotationFileName=None):
        ...
        # Can add differrent annotation formats here
        # print(shapes)    
        annotationFile = os.path.dirname(annotationFileName).split('/')[-1]
        imgFileName = os.path.basename(annotationFileName)
        # savedFileName = os.path.splitext(imgFileName)[0]

        print(annotationFileName)
        with open(os.path.join(annotationFilePath, annotationFile + '.txt'), 'a+') as f:
             bndbox = LabelFile.convertPoints2BndBox(shapes[0]['points'])
             f.write(imgFileName+ ' ') 
             ss = ' '.join(map(lambda x:str(x).rjust(4, ' '), [ bndbox[0], bndbox[1], bndbox[2], bndbox[3] ]))  + '\n'
             f.write(ss)
    ...
    def openNextImg(self, _value=False):
        ...
        #     for i in self.shape:
        #         fff.write(imgFileName, shape['points'])
    ...
    def _saveFile(self, annotationFilePath):
        pass
        # if annotationFilePath and self.saveLabels(annotationFilePath, self.filePath):
        #     self.setClean()
